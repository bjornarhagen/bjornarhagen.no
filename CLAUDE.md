...
Personal site for Bjørnar Hagen. Astro 5 (server output) + Bun runtime + Tailwind v4, deployed as a container on Hetzner (see DEPLOYMENT.md).
...
-   **Astro 5** with `output: "server"` and the standalone Node adapter for container builds (`BUILD_TARGET=container` in `astro.config.ts`)
